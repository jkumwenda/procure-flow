<template>
  <div class="flex flex-col space-y-1 font-roboto-light rounded-xl text-catskill-white-50 bg-catalina-blue-500 px-4 py-2">
    <p class="font-bold text-md">Edit supplier category</p>
    <p class="text-sm font-roboto-light text-catskill-white-800">
      ...
      <span>Edit supplier category</span>
    </p>
  </div>
  <div class="flex flex-1 bg-catskill-white-100 p-4 rounded-xl shadow-sm">
    <form class="flex flex-col w-5/12 space-y-4" @submit.prevent="editSupplierCategory" method="POST">
      <label class="block">
        <span class="after:content-['*'] after:ml-0.5 after:text-red-500 block text-sm font-medium text-slate-700">
          Supplier category
        </span>
        <input type="input" name="supplier_category" v-model="supplierCategoryData.supplier_category"
      ...
      <label class="block">
        ...
        <textarea type="textarea" name="description" v-model="supplierCategoryData.description"
          class="mt-2 px-3 py-3 bg-white border border-1 shadow-sm border-silver-600 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-xl sm:text-sm focus:ring-1"
      ...
      <div class="flex flex-row space-x-4">
        ...
        <router-link :to="{ name: 'SupplierCategories' }"
...
export default {
  ...
  methods: {
    async editSupplierCategory() {
      this.isLoading = true;
      try {
        const response = await updateItem(
          "supplier_categories",
          this.id,
          this.supplierCategoryData
        );
        this.supplier_category = response.data;
        this.isLoading = false;
        router.push("/supplier-categories");
      } catch (error) {
        console.error("Error fetching  supplier_category:", error);
        this.isLoading = false;
      }
    },
    async getSupplierCategory() {
      try {
        const response = await fetchItem("supplier_categories", this.id);
        this.supplierCategoryData.supplier_category = response.supplier_category;
        this.supplierCategoryData.description = response.description;
        this.isLoading = false;
      } catch (error) {
