<template>
  <div class="flex flex-col space-y-1 font-roboto-light rounded-xl text-catskill-white-50 bg-catalina-blue-500 px-4 py-2">
    <p class="font-bold text-md">Add unit of measure</p>
  ...
  <div class="flex flex-1 bg-catskill-white-100 p-4 rounded-xl shadow-sm">
    <form class="flex flex-col w-5/12 space-y-4" @submit.prevent="addUnitOfMeasure" method="POST">
      <label class="block">
...
export default {
  ...
  methods: {
    ...
    async addUnitOfMeasure() {
      this.isLoading = true;
      try {
        const response = await createItem(
          "unit_of_measures",
          this.unitOfMeasureData
        );
