<template>
  <div class="flex flex-col space-y-1 font-roboto-light rounded-xl text-catskill-white-50 bg-catalina-blue-500 px-4 py-2">
    <p class="font-bold text-md">Edit financial year</p>
    <p class="text-sm font-roboto-light text-catskill-white-800">
      ...
      <span>Edit financial year</span>
    </p>
  </div>
  <div class="flex flex-1 bg-catskill-white-100 p-4 rounded-xl shadow-sm">
    <form class="flex flex-col w-5/12 space-y-4" @submit.prevent="editFinancialYear" method="POST">
      <label class="block">
...
export default {
  ...
  methods: {
    formatDateForSubmit() {
      ...
      const formattedEndDate = this.financialYearData.end_date.toISOString().split('T')[0];
    ...
    async editFinancialYear() {
      this.formatDateForSubmit();
      console.log('Start Date:', this.financialYearData.start_date);
      console.log('End Date:', this.financialYearData);
      ...
      try {
        ...
        this.financial_year = response.data;
    ...
    async getFinancialYear() {
      try {
        const response = await fetchItem("financial_years", this.id);
        this.financialYearData.financial_year = response.financial_year;
        this.financialYearData.start_date = this.formatReceivedDate(response.start_date);
        this.financialYearData.end_date = this.formatReceivedDate(response.end_date);
        this.financialYearData.status = response.status;
        this.isLoading = false;
      } catch (error) {
