<template>
  <div
    ...
    <p class="font-bold text-md">Edit budget</p>
  ...
  <div class="flex flex-1 bg-catskill-white-100 p-4 rounded-xl shadow-sm">
    <form class="flex flex-col w-5/12 space-y-4" @submit.prevent="editBudget" method="POST">
      <label class="block">
...
export default {
  ...
  methods: {
    ...
    async editBudget() {
      this.isLoading = true;
      try {
        const response = await updateItem("budgets", this.id, this.budgetData);
    ...
    async getBudget() {
      try {
        const response = await fetchItem("budgets", this.id);
        this.budgetData.amount = response.amount;
        this.budgetData.department_id = response.department_id;
        this.budgetData.financial_year_id = response.financial_year_id;
        this.isLoading = false;
      } catch (error) {
        console.error("Error fetching budgets:", error);
        this.isLoading = false;
