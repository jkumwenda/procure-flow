<template>
  <div class="flex flex-col space-y-1 font-roboto-light rounded-xl text-catskill-white-50 bg-catalina-blue-500 px-4 py-2">
    <p class="font-bold text-md">Edit approval action</p>
    <p class="text-sm font-roboto-light text-catskill-white-800">
      ...
      <span>Edit approval action</span>
    </p>
  </div>
  <div class="flex flex-1 bg-catskill-white-100 p-4 rounded-xl shadow-sm">
    <form class="flex flex-col w-5/12 space-y-4" @submit.prevent="editApprovalAction" method="POST">
      <label class="block">
...
export default {
  ...
  mounted() {
    this.getApprovalAction();
  },
  methods: {
    async editApprovalAction() {
      this.isLoading = true;
      try {
        ...
        this.approval_action = response.data;
        this.isLoading = false;
        router.push("/approval-actions");
      } catch (error) {
        console.error("Error fetching  approval_action:", error);
        this.isLoading = false;
      }
    },
    async getApprovalAction() {
      try {
        const response = await fetchItem("approval_actions", this.id);
        this.approvalActionData.approval_action = response.approval_action;
        this.approvalActionData.description = response.description;
        this.isLoading = false;
      } catch (error) {
