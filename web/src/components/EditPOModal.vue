<template>
  <div v-if="show" class="fixed inset-0 flex items-center justify-center z-50">
    <div
      ...
      <form v-else class="flex flex-col space-y-4 p-4 px-6 space-y-4" @submit.prevent="editPurchaseOrder" method="POST">
        <label class="block">
...
export default {
  ...
  methods: {
    ...
    loadData(newSupplier) {
      this.requestSupplierData = {
    ...
    async editPurchaseOrder() {
      console.log("koma guys", this.requestSupplierData);
      this.isLoading = true;
      try {
        const response = await updateItem("purchase_orders", this.purchaseOrder.id, this.requestSupplierData);
        this.purchase_order = response.data;
        this.isLoading = false;
        this.message = "Purchase order edited successfully";
        this.$emit("confirmed");
      } catch (error) {
