<template>
  <div v-if="show" class="fixed inset-0 flex items-center justify-center z-50">
    <div
      ...
      <form v-else class="flex flex-col block space-y-2 p-4 " @submit.prevent="addApprovalWorkflow" method="POST">
        <label class="block">
...
export default {
  ...
  methods: {
    async addApprovalWorkflow() {
      ...
      this.approvalWorkflowData.workflow_id = this.workflow_id
      try {
        const response = await createItem("workflows/approval_workflows/", this.approvalWorkflowData);
        this.approval_workflow = response;
        this.isLoading = false;
        this.message = "Request approval workflow added successfully"
