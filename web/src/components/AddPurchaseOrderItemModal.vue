<template>
  <div v-if="show" class="fixed inset-0 flex items-center justify-center z-50">
    <div
      ...
      <form class="flex flex-col space-y-4 p-4 px-6 space-y-4" @submit.prevent="addPurchaseOrderItem" method="POST">
...
export default {
  ...
  methods: {
    ...
    async addPurchaseOrderItem() {
      ...
      try {
        const response = await createItem("purchase_orders/purchase_order_items/", this.purchaseOrderItemData);
        this.request_item = response.data;
        this.isLoading = false;
        this.message = "Purchase order item added successfully"
