<template>
  <div v-if="show" class="fixed inset-0 flex items-center justify-center z-50">
    <div
      ...
      <form v-else class="flex flex-col block space-y-2 p-4 " @submit.prevent="AddRejectRequest" method="POST">
        <label class="block">
          <textarea type="text" rows="6" name="comment" v-model="rejectRequestData.comment"
...
export default {
  ...
  methods: {
    async AddRejectRequest() {
      this.isLoading = true; close
      this.rejectRequestData.request_id = this.request_id
      try {
        const response = await createItem("requests/request_reject_history/", this.rejectRequestData);
        this.reject_request = response;
        this.isLoading = false;
        this.message = "Request rejected request successfully"
