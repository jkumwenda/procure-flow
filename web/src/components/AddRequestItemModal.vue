<template>
  <div v-if="show" class="fixed inset-0 flex items-center justify-center z-50">
    <div
      ...
      <form class="flex flex-col space-y-4 p-4 px-6 space-y-4" @submit.prevent="addRequestItem" method="POST">
...
export default {
  ...
  methods: {
    ...
    async addRequestItem() {
      ...
      try {
        const response = await createItem("requests/request_items/", this.requestItemData);
        this.request_item = response.data;
        this.isLoading = false;
        this.message = "Request item added successfully"
